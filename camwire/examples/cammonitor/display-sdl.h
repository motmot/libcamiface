...
/***********************************************************************
    ...
    Description:
    This module uses SDL to show images on the screen.
...
Display_info display_sdl_create(const int width, const int height,
...
/* Sets up and displays a display interface window of the given size (in
   ...
   display_sdl_destroy() must be called when done to free the allocated
   memory. */

void display_sdl_destroy(Display_info info);
/* Frees the memory allocated by display_sdl_create(). */
