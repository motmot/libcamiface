...
#define CAM_IFACE_API_VERSION "20061205a"
...
CAM_IFACE_API void cam_iface_get_camera_info(int index, Camwire_id *out_camid);
...
typedef struct CamContext CamContext;
struct CamContext { // These are READ ONLY. To change, call the appropriate CamContext_set function.
...
CAM_IFACE_API CamContext *new_CamContext(int device_number,int NumImageBuffers,
					 int mode_number);
CAM_IFACE_API void delete_CamContext(CamContext *ccntxt);
